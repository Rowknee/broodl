...
export default function Login() {
  ...
  const [authenticiating, setAuthenticiating] = useState(false)
  ...
  return (
    <div className='flex flex-col flex-1 justify-center items-center gap-4 '>
      ...
      <Button clickHandler={handleSubmit} text={authenticiating ? 'Submitting ' : "Submit"} full />
      </div>
        <p className='text-center'>{isRegister ? 'Already Have An Account? ' : 'Don\'t have an account? ' }<button onClick={() => setIsRegister(!isRegister)} className='text-indigo-600'>{isRegister ? ' Login In ' : ' Register '}</button></p>
